"use strict";
const ENDPOINT = "http://10.33.103.9:5000/current";
const POLL_MS = 2000;
function $(id) {
    return document.getElementById(id);
}
function safeNum(value, invalids = ["UNAVAILABLE", "-9999"]) {
    if (value == null)
        return null;
    if (invalids.includes(value))
        return null;
...
function findByTagAndName(doc, tag, name) {
...
async function fetchXML(signal) {
...
function setBadge(el, value) {
    ...
    if (el.id === "estop") {
        ...
        if (value === "READY" || value === "INACTIVE")
            el.classList.add("ok");
    }
    if (el.id === "exec") {
        if (value === "ACTIVE" || value === "EXECUTING")
            el.classList.add("ok");
        else if (value === "STOPPED" || value === "INTERRUPTED")
            el.classList.add("warn");
    }
}
function formatMaybeNumber(n, decimals = 3) {
